...
%% Load the data 
DataLakeFolder= {'C:\Users\Yi.Li\OneDrive - Fortescue Zero Ltd\General - Battery Performance Team\YL\DataWarehouse\RaceData\Michelotto'};
% FileName= 'Monza 30C\230718164401.mat';
% FileName= '240426142416.mat'
FileName= '240511225928_SpaD3.mat';
...
TimeStart= 60098;
 TimeEnd=61239.2;
% TimeEnd=  max(data.SecondsIntoExport);

idxSelectData = find(data.SecondsIntoExport< TimeEnd&data.SecondsIntoExport> TimeStart);

TestData= TestDataHelper.selectData(data,idxSelectData); 

fig=figure('Color','White','Position',[280,7,1500,1000]);
fig=PlotHelper.PlotExpData(fig, TestData)
sgtitle(strrep(FileName ,'_','-'))

%% Create power input profile 
% get part of the test data 

PowerInput_Eoln=table();
PowerInput_Eoln.PowerKW=double(TestData.Pack.Power_kW);
SelectTime= TestData.Time_Seconds; 
PowerInput_Eoln.Time=double(SelectTime-SelectTime(1));

CoolantIn.Temperature= double(TestData.Coolant.InletTemperature_degC);
CoolantIn.Time= PowerInput_Eoln.Time;

SOCselected = TestData.Cell.MaxSoc; 
SOCstart           = double(SOCselected(1)/100);
...
figure()
yyaxis left
plot(PowerInput_Eoln.Time, PowerInput_Eoln.PowerKW)
yyaxis right
plot(CoolantIn.Time, CoolantIn.Temperature)
%%  Load Bus
...
for idxCoolR = 1:length(CellPar.ThermalProperties.CoolTIn)
    ...
    for idxCoolFlowRate = 1:length(CellPar.ThermalProperties.CoolFlowRate)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        out = sim('H24_outletEstModel_2cells_VariedCoolantTemp.slx');
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        fig=PlotHelper.PlotSimout_CompareTestData_V2(fig, SimResults,InputPar,...
                                                                                    SOCstart,TestData);
